...
def balanced(m):
    ...
    ll = length(le)
    rl = length(r)
    lt = total_weight(end(le))
    rt = total_weight(end(r))
    return (ll * lt == rl * rt) and balanced(end(le)) and balanced(end(r))
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        tl = totals_tree(end(left(m)))
        tr = totals_tree(end(right(m)))
        return tree(label(tl) + label(tr), [tl, tr])
...
def has_path(t, word):
    ...
    if label(t) != word[0]:
        return False
    if len(word) == 1: #如果两者相等并且word长度等于1
        return True
    for b in branches(t):
