...
class Account:
    ...
    def __repr__(self):
        ...
        cnt1 = 0
        cnt2 = 0
        for a, b in self.transactions:
            if a == 'withdraw':
                cnt2 += 1
            elif a == 'deposit':
                cnt1 += 1
        return f"Accountholder: {self.holder}, Deposits: {cnt1}, Withdraws: {cnt2}"
...
def insert(link, value, index):
    ...
    while cnt != index:
        ...
        cnt += 1
    L = Link(link.first,link.rest)#直接把值修改
    link.first = value
    link.rest = L
